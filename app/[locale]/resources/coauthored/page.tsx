...
import { getCoAuthoredBooks, formatMonographDate } from "@/lib/i18n/resource-data";
...
export async function generateMetadata({
  ...
  return {
    title: `${dict.resources.tabs.coauthored} — Bitcoinology Lab`,
  };
}

export default async function CoAuthoredBooksPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const m = dict.coAuthoredBooks;
  const books = getCoAuthoredBooks(locale);
